...
```{r}
df_sample_2$Task <- ifelse(grepl(1, df_sample_1$Order) & grepl(paste(toMatch1,collapse="|"), df_sample_1$TRIAL_INDEX), "Search", 
                       ifelse(grepl(2, df_sample_1$Order) & grepl(paste(toMatch2,collapse="|"), df_sample_1$TRIAL_INDEX), "Search", "Count"))
...
trial.type: whether the trial is from experiment or control group trial from staircase.
pas: 
trial: trial number
jitter.x:
jitter.y: 
...
obj.resp:
subject:
correct: indicating whether subject was correct or not. Correct = 1, not correct = 0. 
