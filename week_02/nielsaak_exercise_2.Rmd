...
Subject: letter is indicating whether the subject is male (M) or female (F). The number indicates unique participant number.
Gender: letter is indicating whether the subject is male (M) or female (F).
Scenario: the number indicates what specific scenario the subject has been presented with in that observation.
Attitude: indicating whether the scenario presetned was of a "polite" (pol) nature or a "informal" (inf) nature.
Total_duration: duration of trial in seconds (???)
f0mn: frequency or pitch of their voice measured in some unit.
...
model_f1_1 <- lm(f0mn~scenario, data = politeness_f1)
model_f1_2 <- lm(f0mn~as.factor(scenario), data = politeness_f1)
