...
```{r}
#the dataset used in this model consists of subject id, binary gender indication (F or M), scenario index (from 1 to 7 depending on what the scenario was), a variable indicating whether the text should be spoken in an formal/polite or informal tone, and a variable called f0mn basically stating the average frequency of the utterance. Besides this data on total duration of utterance and numbers of "hisses" was also counted but not relevant for the following model. 
```
